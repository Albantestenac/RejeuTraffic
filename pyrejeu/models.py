...
class Cone(Base):
    ...
    last_version = Column(Integer)                           # Version la plus récente

    # déclaration des relations
    flight = relationship("Flight", back_populates="cones")

    def __repr__(self):
        return"<Cone(pos_x=%f, pos_y=%f, vit_x=%d, vit_y=%d, flight_level=%d, rate=%f, tendency=%d, hour=%d, flight=%d, version=%d, last_version=%d)>" % \
              (self.pos_x, self.pos_y, self.vit_x, self.vit_y, self.flight_level, self.rate, self.tendency, self.hour, self.flight.id, self.version, self.last_version)
